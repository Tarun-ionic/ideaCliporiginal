...
import {loginMethods, charityData, stringKeys, userType} from '../../../utilities/constant';
...
export default function CharityProfileInitialize() {
    const scroll = useRef();
    ...
    const [user, setUser] = useState({...charityData});
    const [originalData, setOriginalData] = useState({...charityData});
    const [saving, setSaving] = useState(false);
    const [ACNErrors, setACNErrors] = useState(false);
    ...
    const [userVerified, setUserVerified] = useState(isEdit || false);
    const [emailVerified, setEmailVerified] = useState(isEdit || false);
    const [ACNVerified, setACNVerified] = useState(true);
    const [referralCode, setReferralCode] = useState('');

    const emptyMessage = {
        orgName: 'Charity name field should not be empty.',
    ...
    useBackHandler(() => {
        onBackPress();
        return route.name === 'InitialSetupCharity';
    }, []);
        ...
        apollo
            ...
            .then(({data}) => {
                if (data) {
                    ...
                    convertCharityData(data?.profile);
                }
            })
            .catch();
    };
    ...
    const imageUploaded = (status, name, thumb) => {
        if (status) {
            setUser({
                ...user,
                profileImage: 'dp/' + route.params.user.uid + '/' + name + '.jpg',
    ...
    function setSelectedValue(label, value) {
        if (label === 'ethnicCommunity') {
            ...
            if (value && value?.length > 0) {
                let activityList = user?.activityList ? [...user.activityList] : [];
                let index = activityList.indexOf(value);
                if (index >= 0) {
                    activityList.splice(index, 1);
                    setUser({...user, activityList: activityList});
                } else {
                    activityList.push(value);
                    setUser({...user, activityList: activityList});
                }
            }
            // else if (label === errorKey.ACN) {
            //     setACNErrors(value);
            // }
        }
            ...
            setUser({...user, [label === 'ABN' ? 'ACN' : label]: value});
        }
    }
    ...
    const updateUser = () => {
        ...
        apollo
            ...
            .mutate({
                ...
                mutation: mutations.updateCharity,
            })
            .then(() => {
    ...
    function checkFormData() {
        if (!user.orgName) {
            Toast.show(strings.enter_charity_name);
        } else if (nicknameError || !userVerified) {
            Toast.show('Enter a unique username');
        } else if (!user.displayName) {
            Toast.show('Enter a unique username');
        } else if (emailError) {
            Toast.show('Enter a unique email address');
        } else if (!user.email || !emailVerified) {
            Toast.show('Enter a unique email address');
        }
        // else if (ACNErrors || !ACNVerified) {
    ...
    const changeTab = _tab => {
        setCurrentTab(currentTab === _tab ? '' : _tab);
    };
                        ...
                        {currentTab === 'basicInfo' && (
                            <View style={styles.tabContent}>
                                ...
                                <AuthenticatingTextBox
                                    label={strings.registered_charity_account}
                                    value={user.orgName}
                                    setValue={setSelectedValue}
                                ...
                                />
                                <ABNTextBox
                                    verified={setACNVerified}
                                    value={user.ACN}
                                    setValue={setSelectedValue}
                                    originalValue={originalData.ACN}
                                    labelKey={'ABN'}
                                />

                                <MainActivitiesAutofillDropdown
                                    value={user.activityList}
                                    setValue={setSelectedValue}
                        ...
                        {currentTab === 'contactInfo' && (
                            <View style={styles.tabContent}>
                                ...
                                <Phonenumber
                                    ...
                                    labelKey={'Charity phone number'}
                                    phoneCode={user.orgMobile.phoneCode}
                                />

                                {/*Place picker*/}
                                <View style={styles.minMapContainer}>
                                    <Text style={styles.labelTextView}>Charity/NFP location</Text>
